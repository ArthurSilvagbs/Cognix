...
Instruções para o Claude Code neste repositório. Fonte versionada no vault Obsidian
...
- Feche a mensagem com `Co-Authored-By: Claude <noreply@anthropic.com>`.
- Nunca `--no-verify` nem force push sem pedido explícito.

## Agente de implementação (opcional)

Existe um agente `cognix-developer` (`.claude/agents/cognix-developer.md`) com o método
...
Ao longo do caminho, ensine na prática: gestão de contexto (vault + CLAUDE.md), agentes
